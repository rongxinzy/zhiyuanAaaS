...
type ControlEventType =
  (typeof ControlEventType)[keyof typeof ControlEventType];
...
type ControlEventScopeType =
  (typeof ControlEventScopeType)[keyof typeof ControlEventScopeType];
...
type ControlEventResourceType =
  (typeof ControlEventResourceType)[keyof typeof ControlEventResourceType];
...
const CONTROL_EVENT_RESOURCES: Record<
  ControlEventType,
  ControlEventResourceType
> = {
...
const CONTROL_EVENT_RESOURCE_LABELS: Record<
  ControlEventResourceType,
  AdminTranslationKey
> = {
...
export function Events({
  ...
  const canWrite = hasAdminPermission(identity, AdminPermission.EventsWrite);
  ...
  const [controlEvents, setControlEvents] = useState<
    readonly AdminControlEvent[]
  >([]);
  const [controlNextCursor, setControlNextCursor] = useState<string | null>(
    null,
  );
  ...
  const loadControlEvents = useCallback(
    async (filters: EventFilters = {}, append = false) => {
      ...
      try {
        ...
        notify(
          ...
          translate(language, "eventsFailed"),
        );
      } finally {
  ...
  const search = async (event?: FormEvent) => {
    ...
    try {
      ...
      setStatus("error");
      notify(AdminNotificationKind.Error, translate(language, "eventsFailed"));
    }
  };
  ...
  const loadMoreAudit = async () => {
    ...
    setStatus("loading");
    try {
      const page = await client.searchAudit({
        ...auditFilters,
      ...
      <div className="mx-auto flex w-full max-w-4xl flex-col gap-6">
        <div>
          ...
          <p className="mt-1.5 text-sm text-muted-foreground">
            {translate(language, "eventsDescription")}
          </p>
        </div>
        <div className="grid grid-cols-1 gap-4 lg:grid-cols-2">
          {canWrite ? (
            <ControlEventPublishCard
        ...
        <ControlEventList
          ...
          nextCursor={controlNextCursor}
          canWrite={canWrite}
          client={client}
          onChanged={() => loadControlEvents(controlFilters)}
          onLoadMore={() =>
            loadControlEvents(
              { ...controlFilters, cursor: controlNextCursor ?? "" },
              true,
            )
          }
          onSearch={searchControlEvents}
          onError={() => {
            setControlEventsError(true);
            notify(
              AdminNotificationKind.Error,
...
function ControlEventPublishCard({
  pending,
  published,
  onPublish,
}: {
  ...
  readonly onPublish: (input: JsonObject) => Promise<void>;
}) {
  const [type, setType] = useState<ControlEventType>(
    ControlEventType.ModelCatalogChanged,
  );
  const [scopeType, setScopeType] = useState<ControlEventScopeType>(
    ControlEventScopeType.Global,
  );
  const [scopeId, setScopeId] = useState("");
  const [resourceType, setResourceType] = useState<ControlEventResourceType>(
    ControlEventResourceType.Model,
  );
  ...
  const submit = async (event: FormEvent<HTMLFormElement>) => {
    ...
    if (
      (scopeType !== ControlEventScopeType.Global && !scopeId.trim()) ||
      !Number.isInteger(ttl) ||
      ttl < 1 ||
      ttl > 1440 ||
      (hasResource && (!resourceId.trim() || !resourceRevision.trim()))
    ) {
      setFailed(true);
      notify(
        AdminNotificationKind.Error,
        translate(language, "eventFormFailed"),
      );
      return;
    }
    setFailed(false);
    const input: JsonObject = {
      ...
      scope:
        scopeType === ControlEventScopeType.Global
          ? { type: scopeType }
          : { type: scopeType, id: scopeId.trim() },
      ...
      ...(hasResource
        ? {
            resource: {
              type: resourceType,
              id: resourceId.trim(),
              revision: resourceRevision.trim(),
            },
          }
        : {}),
    ...
    <Card>
      <CardHeader>
        <div className="flex items-center gap-2">
          ...
          <CardTitle>{translate(language, "publishEvent")}</CardTitle>
        </div>
      </CardHeader>
      <CardContent>
        <form className="flex flex-col gap-4" noValidate onSubmit={submit}>
          ...
          <FieldGroup>
            <Field>
              <FieldLabel>{translate(language, "eventType")}</FieldLabel>
              <ToggleGroup
                value={[type]}
                onValueChange={(next) => {
                  if (next[0]) {
                    const nextType = next[0] as ControlEventType;
                    setType(nextType);
                    setResourceType(CONTROL_EVENT_RESOURCES[nextType]);
                  }
                }}
                variant="outline"
                className="flex-wrap"
                aria-label={translate(language, "publishEventType")}
              >
                <ToggleGroupItem value={ControlEventType.SkillManifestChanged}>
                  {translate(language, "skillManifestChanged")}
                </ToggleGroupItem>
                <ToggleGroupItem value={ControlEventType.PluginManifestChanged}>
                  {translate(language, "pluginManifestChanged")}
                </ToggleGroupItem>
                <ToggleGroupItem
                  value={ControlEventType.CredentialAssignmentsChanged}
                >
                  {translate(language, "credentialAssignmentsChanged")}
                </ToggleGroupItem>
                <ToggleGroupItem value={ControlEventType.ModelCatalogChanged}>
                  {translate(language, "modelCatalogChanged")}
                </ToggleGroupItem>
              </ToggleGroup>
            </Field>
            <Field>
              <FieldLabel>{translate(language, "scopeType")}</FieldLabel>
              <ToggleGroup
                value={[scopeType]}
                onValueChange={(next) => {
                  if (next[0]) {
                    const nextScope = next[0] as ControlEventScopeType;
                    setScopeType(nextScope);
                    if (nextScope === ControlEventScopeType.Global)
                      setScopeId("");
                  }
                }}
                variant="outline"
                aria-label={translate(language, "publishScopeType")}
              >
                <ToggleGroupItem value={ControlEventScopeType.Global}>
                  {translate(language, "globalScope")}
                </ToggleGroupItem>
                <ToggleGroupItem value={ControlEventScopeType.Team}>
                  {translate(language, "teamScope")}
                </ToggleGroupItem>
                <ToggleGroupItem value={ControlEventScopeType.User}>
                  {translate(language, "userScope")}
                </ToggleGroupItem>
              </ToggleGroup>
            </Field>
            {scopeType !== ControlEventScopeType.Global ? (
            ...
            <FieldGroup className="grid grid-cols-1 gap-4 sm:grid-cols-2">
              ...
              <Field>
                <FieldLabel htmlFor="publish-supersedes-key">
                  {translate(language, "publishSupersedesKey")}
                </FieldLabel>
                <Input
                  id="publish-supersedes-key"
                  value={supersedesKey}
                  onChange={(event) => setSupersedesKey(event.target.value)}
                  placeholder={translate(language, "optional")}
                  disabled={pending}
                />
              </Field>
    ...
    <Card>
      <CardHeader>
        <div className="flex items-center gap-2">
          ...
          <CardTitle>{translate(language, "auditSearch")}</CardTitle>
        </div>
      </CardHeader>
      <CardContent>
          ...
          <FieldGroup className="grid grid-cols-1 gap-4 sm:grid-cols-2">
            <Field>
              ...
              <Input
                ...
                placeholder={translate(language, "eventTypePlaceholder")}
              />
            </Field>
            <Field>
              <FieldLabel htmlFor="event-user-id">
                {translate(language, "eventUserId")}
              </FieldLabel>
              <Input
                id="event-user-id"
            ...
            <Field>
              <FieldLabel htmlFor="event-resource-type">
            ...
            <Field>
              <FieldLabel htmlFor="event-resource-id">
                {translate(language, "eventResourceId")}
              </FieldLabel>
              <Input
                id="event-resource-id"
                value={String(filters.resourceId ?? "")}
                onChange={(event) => update("resourceId", event.target.value)}
              />
            </Field>
            <Field>
              <FieldLabel htmlFor="event-result">
                {translate(language, "eventResult")}
              </FieldLabel>
              <Input
                id="event-result"
...
function ControlEventList({
  ...
  readonly events: readonly AdminControlEvent[];
  readonly loading: boolean;
  ...
  readonly onSearch: (filters: EventFilters) => Promise<void>;
  ...
  if (loading)
    return (
      <Card>
        <CardHeader>
          <CardTitle>{translate(language, "controlEvents")}</CardTitle>
        </CardHeader>
        <CardContent>
          <p className="text-sm text-muted-foreground">
    ...
    return (
      <Empty>
        <EmptyHeader>
          ...
          <EmptyTitle>{translate(language, "controlEventsEmpty")}</EmptyTitle>
          <EmptyDescription>
            {translate(language, "controlEventsEmptyHint")}
          </EmptyDescription>
        </EmptyHeader>
      </Empty>
    );
    ...
    <Card>
      <CardHeader>
        <div className="flex flex-col gap-3">
          ...
          <ControlEventFilter onSearch={onSearch} disabled={loading} />
        </div>
      </CardHeader>
      <CardContent>
        <div className="overflow-hidden rounded-lg border border-border bg-card">
          <Table>
            <TableHeader>
              <TableRow>
                <TableHead>{translate(language, "eventType")}</TableHead>
                <TableHead>{translate(language, "scope")}</TableHead>
                <TableHead>{translate(language, "status")}</TableHead>
                <TableHead>{translate(language, "time")}</TableHead>
...
function ControlEventFilter({
  ...
  const submit = (event: FormEvent<HTMLFormElement>) => {
    event.preventDefault();
    void onSearch(withoutEmpty(filters));
  };
  ...
  return (
    <form className="grid grid-cols-1 gap-2 sm:grid-cols-2" onSubmit={submit}>
      <Field>
        <FieldLabel htmlFor="control-event-type">
          {translate(language, "eventType")}
        </FieldLabel>
        <Input
          id="control-event-type"
          value={String(filters.type ?? "")}
          onChange={(event) => update("type", event.target.value)}
        />
      </Field>
      <Field>
        <FieldLabel htmlFor="control-scope-type">
          {translate(language, "scopeType")}
        </FieldLabel>
        <Input
          id="control-scope-type"
          value={String(filters.scopeType ?? "")}
          onChange={(event) => update("scopeType", event.target.value)}
        />
      </Field>
      <Field>
        <FieldLabel htmlFor="control-scope-id">
          {translate(language, "scopeId")}
        </FieldLabel>
        <Input
          id="control-scope-id"
          value={String(filters.scopeId ?? "")}
          onChange={(event) => update("scopeId", event.target.value)}
        />
      </Field>
      <Field>
        <FieldLabel htmlFor="control-created-after">
          {translate(language, "createdAfter")}
        </FieldLabel>
        <Input
          id="control-created-after"
          type="datetime-local"
          value={dateTimeInputValue(filters.createdAfter)}
          onChange={(event) =>
            update(
              "createdAfter",
              event.target.value
      ...
      <Field>
        <FieldLabel htmlFor="control-created-before">
          {translate(language, "createdBefore")}
        </FieldLabel>
        <Input
          id="control-created-before"
          type="datetime-local"
          value={dateTimeInputValue(filters.createdBefore)}
          onChange={(event) =>
            update(
              "createdBefore",
              event.target.value
...
function ControlEventRow({
  ...
  readonly event: AdminControlEvent;
  readonly canWrite: boolean;
  readonly client: AdminConsoleClient;
  readonly onChanged: () => Promise<void>;
  readonly onError: () => void;
  ...
  const inspect = async () => {
    setPending(true);
    try {
      setDetail(await client.getControlEvent(event.eventId));
      setDetailOpen(true);
    } catch {
      onError();
    } finally {
  ...
  const cancel = async () => {
    setPending(true);
    try {
      await client.cancelControlEvent(event.eventId);
      await onChanged();
    } catch {
      onError();
    } finally {
  ...
  return (
    <>
      <TableRow>
        <TableCell className="max-w-48 truncate font-normal">
          {event.type}
        </TableCell>
        ...
        <TableCell>
          ...
          >
            {translate(language, eventStateLabel(event.state))}
          </Badge>
        </TableCell>
        <TableCell className="text-xs text-tertiary-foreground">
        ...
        <TableCell>
          <div className="flex justify-end gap-1.5">
            <Button
              size="icon-xs"
              variant="ghost"
              aria-label={translate(language, "viewDetails")}
              title={translate(language, "viewDetails")}
              disabled={pending}
              onClick={() => void inspect()}
            >
              <Eye />
            </Button>
            {canWrite && event.state === "active" ? (
              <AlertDialog>
                <AlertDialogTrigger
                  render={
                    <Button
                      ...
                      aria-label={translate(language, "cancelEvent")}
                      title={translate(language, "cancelEvent")}
                      disabled={pending}
                    />
                  }
                >
                ...
                <AlertDialogContent size="sm">
                  <AlertDialogHeader>
                    <AlertDialogTitle>
                      {translate(language, "cancelEventTitle")}
                    </AlertDialogTitle>
                    <AlertDialogDescription>
                      {translate(language, "cancelEventDescription")}
                    </AlertDialogDescription>
                  </AlertDialogHeader>
                  <AlertDialogFooter>
                    ...
                    <AlertDialogAction
                      ...
                      onClick={() => void cancel()}
                    >
                      {translate(language, "confirmCancelEvent")}
                    </AlertDialogAction>
                  </AlertDialogFooter>
                </AlertDialogContent>
              </AlertDialog>
      ...
      <Dialog open={detailOpen} onOpenChange={setDetailOpen}>
        <DialogContent className="sm:max-w-lg">
          <DialogHeader>
            <DialogTitle>{translate(language, "eventDetails")}</DialogTitle>
            <DialogDescription>
              {translate(language, "eventDetailsDescription")}
            </DialogDescription>
          </DialogHeader>
...
function AuditList({
  ...
  readonly records: readonly AdminEventRecord[];
  readonly nextCursor: string | null;
  readonly loading: boolean;
  ...
  const loadDelivery = async (cursor?: string, append = false) => {
    ...
    try {
      const page = await client.deliverySummary(
        eventId,
    ...
    return (
      <Empty>
        <EmptyHeader>
          ...
          <EmptyTitle>{translate(language, "auditEmpty")}</EmptyTitle>
          <EmptyDescription>
            {translate(language, "auditEmptyHint")}
          </EmptyDescription>
        </EmptyHeader>
      </Empty>
    );
  return (
    <div className="flex flex-col gap-3">
        ...
        <Table>
          <TableHeader>
            <TableRow>
              <TableHead>{translate(language, "eventType")}</TableHead>
              <TableHead>{translate(language, "scope")}</TableHead>
              <TableHead>{translate(language, "userId")}</TableHead>
              <TableHead>{translate(language, "result")}</TableHead>
              <TableHead className="text-right">
                {translate(language, "time")}
              </TableHead>
            </TableRow>
          </TableHeader>
          <TableBody>
            {records.map((record, index) => (
              <TableRow key={record.eventId ?? index}>
                <TableCell className="font-normal">
                  {record.type || translate(language, "unknownEvent")}
                </TableCell>
                <TableCell className="text-tertiary-foreground">
                  {record.scopeType
                    ? `${record.scopeType}${record.scopeId ? ` / ${record.scopeId}` : ""}`
                    : translate(language, "global")}
                </TableCell>
                <TableCell className="max-w-40 truncate text-xs text-tertiary-foreground">
                  {record.userId || translate(language, "notProvided")}
                </TableCell>
                <TableCell>
                  <Badge
                    variant={
                  ...
                  >
                    {record.result || translate(language, "notProvided")}
                  </Badge>
                </TableCell>
                <TableCell className="text-right text-xs text-tertiary-foreground">
                  {record.receivedAt || record.createdAt
                    ? formatTimestamp(record.receivedAt ?? record.createdAt)
      ...
      {eventId ? (
        ...
        >
          {deliveryLoading ? <Spinner data-icon="inline-start" /> : null}
          {translate(language, "viewDelivery")}
        </Button>
      ) : null}
      {delivery ? (
        <div className="flex flex-col gap-2">
          <div className="overflow-hidden rounded-lg border border-border bg-card">
            <Table>
              <TableHeader>
                <TableRow>
                  <TableHead>{translate(language, "sessionId")}</TableHead>
                  <TableHead>{translate(language, "deliveryState")}</TableHead>
                  <TableHead>{translate(language, "attempts")}</TableHead>
                  <TableHead>{translate(language, "deliveryError")}</TableHead>
                </TableRow>
              </TableHeader>
              <TableBody>
                {delivery.items.map((item) => (
                  <TableRow key={item.deliveryId}>
                    <TableCell className="max-w-48 truncate text-xs">
                      {item.sessionId || item.deliveryId}
                    </TableCell>
                    <TableCell>
                      <Badge
                        variant={
          ...
          {deliveryCursor ? (
            <Button
              ...
              className="self-start"
              disabled={deliveryLoading}
              onClick={() => void loadDelivery(deliveryCursor, true)}
            >
              {translate(language, "loadMore")}
            </Button>
          ) : null}
