...
function AuditList({ records, eventId, client }: { readonly records: readonly AdminEventRecord[]; readonly eventId: string; readonly client: AdminConsoleClient }) {
  ...
  if (records.length === 0 && !eventId) return <div className="flex flex-col items-center gap-3 rounded-lg border border-dashed bg-background p-10 text-center"><div className="flex size-10 items-center justify-center rounded-lg bg-muted"><ClipboardList className="size-4 text-muted-foreground" aria-hidden="true" /></div><div><p className="text-sm font-medium">{translate('zh', 'auditEmpty')}</p><p className="mt-1 text-xs text-muted-foreground">{translate('zh', 'auditEmptyHint')}</p></div></div>;
