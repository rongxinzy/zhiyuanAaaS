...
function AuditList({ records, eventId, client }: { readonly records: readonly AdminEventRecord[]; readonly eventId: string; readonly client: AdminConsoleClient }) {
  ...
  return <div className="flex flex-col gap-3"><div className="overflow-hidden rounded-lg border bg-background" role="table">{records.map((record, index) => <div className="flex flex-wrap items-center justify-between gap-3 border-b p-4 last:border-b-0" role="row" key={record.eventId ?? index}><div className="min-w-0"><div className="truncate text-sm font-medium">{record.type || translate('zh', 'unknownEvent')}</div><div className="truncate text-xs text-muted-foreground">{record.agentId || translate('zh', 'allAgents')}</div></div><div className="text-xs text-muted-foreground">{record.receivedAt || record.createdAt || translate('zh', 'notProvided')}</div></div>)}</div>{eventId ? <Button variant="outline" size="sm" className="self-start" onClick={async () => setDelivery(await client.deliverySummary(eventId))}>{translate('zh', 'viewDelivery')}</Button> : null}{delivery ? <pre className="overflow-auto rounded-lg border bg-muted p-3 text-xs">{JSON.stringify(delivery, null, 2)}</pre> : null}</div>;
}
