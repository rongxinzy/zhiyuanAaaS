...
export function Events({ client }: { readonly client: AdminConsoleClient }) {
  ...
  const search = async (event?: FormEvent) => {
    ...
    try { setRecords(await client.searchAudit({ ...(type ? { type } : {}) })); setStatus('idle'); } catch { setStatus('error'); }
  };
  const publish = async (event: FormEvent<HTMLFormElement>) => {
    event.preventDefault();
    setStatus('loading');
    try { const result = await client.publishControlEvent({ type: 'model.catalog.changed', scope: { type: 'global' }, task: { type: 'model.reconcile' }, expiresAt: new Date(Date.now() + 300_000).toISOString(), supersedesKey: 'admin-console:model-catalog' } as JsonObject); const id = typeof result.eventId === 'string' ? result.eventId : ''; setEventId(id); setPublished(id || 'created'); setStatus('idle'); } catch { setStatus('error'); }
  };
