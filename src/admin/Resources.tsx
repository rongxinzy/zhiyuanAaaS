    ...
    <section className="flex flex-1 flex-col gap-4 overflow-y-auto p-4 sm:p-6">
      <div className="mx-auto flex w-full max-w-5xl flex-col gap-4">
        <div className="flex items-center justify-between gap-4">
          <h1 className="text-lg font-semibold">{translate('zh', resourceTitle(tab))}</h1>
        ...
        {loading && !resources ? <div className="flex items-center gap-2 rounded-lg border p-6 text-sm text-muted-foreground"><Spinner />{translate('zh', 'loadingResources')}</div> : null}
...
function UsersTable({ users, client, onChanged }: { readonly users: readonly PlatformUser[]; readonly client: AdminConsoleClient; readonly onChanged: () => Promise<void> }) {
  ...
  return <div className="overflow-hidden rounded-lg border" role="table">{users.map(user => <div className="flex flex-wrap items-center justify-between gap-3 border-b p-4 last:border-b-0" role="row" key={user.id}><div className="flex min-w-0 items-center gap-3"><UserRound className="size-4 text-muted-foreground" aria-hidden="true" /><div className="min-w-0"><div className="truncate text-sm font-medium">{user.displayName}</div><div className="truncate text-xs text-muted-foreground">{user.username}</div></div></div><div className="flex items-center gap-2"><Badge variant={user.status === 'active' ? 'default' : 'outline'}>{translate('zh', user.status === 'active' ? 'active' : 'disabled')}</Badge><Button size="sm" variant="outline" onClick={async () => { await client.updateUser(user.id, { status: user.status === 'active' ? 'disabled' : 'active' }); await onChanged(); }}>{translate('zh', user.status === 'active' ? 'disable' : 'enable')}</Button></div></div>)}</div>;
}

function AgentsTable({ agents }: { readonly agents: AdminResources['agents'] }) {
  if (agents.length === 0) return <EmptyState label="agentsEmpty" />;
  return <div className="overflow-hidden rounded-lg border" role="table">{agents.map(agent => <div className="flex flex-wrap items-center justify-between gap-3 border-b p-4 last:border-b-0" role="row" key={agent.agentId}><div className="flex min-w-0 items-center gap-3"><Bot className="size-4 text-muted-foreground" aria-hidden="true" /><div className="min-w-0"><div className="truncate text-sm font-medium">{agent.agentId}</div><div className="text-xs text-muted-foreground">{agent.platform} · {agent.agentVersion}</div></div></div><div className="text-right text-xs text-muted-foreground"><div>{translate('zh', 'lastSeen')}</div><div>{formatDate(agent.lastSeenAt)}</div></div></div>)}</div>;
}
...
function SkillsTable({ skills, client, onChanged }: { readonly skills: readonly AdminSkill[]; readonly client: AdminConsoleClient; readonly onChanged: () => Promise<void> }) {
  ...
  return <div className="overflow-hidden rounded-lg border" role="table">{skills.map(skill => <div className="flex flex-wrap items-center justify-between gap-3 border-b p-4 last:border-b-0" role="row" key={skill.id}><div className="flex min-w-0 items-center gap-3"><Boxes className="size-4 text-muted-foreground" aria-hidden="true" /><div className="min-w-0"><div className="truncate text-sm font-medium">{skill.name}</div><div className="truncate text-xs text-muted-foreground">{skill.description || skill.id}</div></div></div><div className="flex items-center gap-2"><Badge variant={skill.enabled ? 'default' : 'outline'}>{translate('zh', skill.enabled ? 'enabled' : 'disabled')}</Badge><Button size="sm" variant="outline" onClick={async () => { await client.updateSkill(skill.id, { enabled: !skill.enabled }); await onChanged(); }}>{translate('zh', skill.enabled ? 'disable' : 'enable')}</Button></div></div>)}</div>;
}
