...
export function Models({ client }: { readonly client: AdminConsoleClient }) {
  ...
  return <section className="flex flex-1 flex-col gap-5 overflow-y-auto p-4 sm:p-6"><div className="mx-auto flex w-full max-w-4xl flex-col gap-5"><div className="flex items-start justify-between gap-4"><div><p className="text-xs text-muted-foreground">{translate('zh', 'workspaceLabel')}</p><h2 className="mt-1 text-lg font-semibold leading-snug">{translate('zh', 'models')}</h2><p className="mt-1.5 text-sm text-muted-foreground">{translate('zh', 'modelsDescription')}</p></div><Button variant="ghost" size="icon" aria-label={translate('zh', 'refresh')} title={translate('zh', 'refresh')} disabled={loading} onClick={() => void load()}>{loading ? <Spinner /> : <RefreshCw />}</Button></div>{error ? <Alert variant="destructive"><CircleAlert aria-hidden="true" /><AlertDescription>{translate('zh', error)}</AlertDescription></Alert> : null}<ModelCreateForm client={client} onCreated={load} />{loading && !state ? <ModelCatalogSkeleton /> : null}{state ? <ModelList models={state.models} assignments={state.assignments} client={client} onChanged={load} onError={() => setError('modelsLoadFailed')} /> : null}</div></section>;
}
...
function ModelList({ models, assignments, client, onChanged, onError }: { readonly models: readonly AdminModel[]; readonly assignments: readonly ModelAssignment[]; readonly client: AdminConsoleClient; readonly onChanged: () => Promise<void>; readonly onError: () => void }) {
  if (models.length === 0) return <Empty><EmptyHeader><EmptyMedia><ShieldCheck aria-hidden="true" /></EmptyMedia><EmptyTitle>{translate('zh', 'modelsEmpty')}</EmptyTitle><EmptyDescription>{translate('zh', 'modelsEmptyHint')}</EmptyDescription></EmptyHeader></Empty>;
...
function ModelRow({ model, assignments, client, onChanged, onError }: { readonly model: AdminModel; readonly assignments: readonly ModelAssignment[]; readonly client: AdminConsoleClient; readonly onChanged: () => Promise<void>; readonly onError: () => void }) {
  ...
  return <Card className="bg-background"><CardHeader><div className="flex flex-wrap items-start justify-between gap-3"><div className="flex min-w-0 items-start gap-3"><Cpu className="mt-1 size-4 shrink-0 text-muted-foreground" aria-hidden="true" /><div className="min-w-0"><CardTitle className="truncate">{model.displayName}</CardTitle><p className="break-all text-xs text-muted-foreground">{model.id} / {model.upstreamModel || model.localModelRef || translate('zh', 'notProvided')}</p></div></div><div className="flex items-center gap-2"><Badge variant={model.enabled ? 'secondary' : 'outline'}>{translate('zh', model.enabled ? 'enabled' : 'disabled')}</Badge>{model.isDefault ? <Badge variant="outline">{translate('zh', 'defaultModel')}</Badge> : null}</div></div></CardHeader><CardContent className="flex flex-col gap-4"><dl className="grid grid-cols-1 gap-3 text-sm sm:grid-cols-2"><div><dt className="text-xs text-muted-foreground">{translate('zh', 'modelEndpoint')}</dt><dd className="break-all font-medium">{model.endpoint || translate('zh', 'notProvided')}</dd></div><div><dt className="text-xs text-muted-foreground">{translate('zh', 'assignments')}</dt><dd className="font-medium">{assignments.length}</dd></div></dl><div className="flex flex-wrap gap-2"><Button size="sm" variant="outline" disabled={pending} onClick={() => void run(async () => { await client.updateModel(model.id, { enabled: !model.enabled }); })}>{translate('zh', model.enabled ? 'disable' : 'enable')}</Button>{!model.isDefault ? <Button size="sm" variant="outline" disabled={pending} onClick={() => void run(async () => { await client.updateModel(model.id, { isDefault: true }); })}>{translate('zh', 'makeDefault')}</Button> : null}</div>{assignments.length > 0 ? <div className="flex flex-col gap-2 border-t pt-3">{assignments.map(assignment => <div className="flex items-center justify-between gap-3 text-sm" key={assignment.id}><span className="truncate text-muted-foreground">{assignment.subject.type}: {assignment.subject.id}</span><Button size="sm" variant="ghost" className="text-destructive hover:bg-destructive/10 hover:text-destructive" disabled={pending} onClick={() => void run(async () => { await client.deleteModelAssignment(assignment.id); })}><Trash2 data-icon="inline-start" />{translate('zh', 'revoke')}</Button></div>)}</div> : null}</CardContent></Card>;
}

function ModelCatalogSkeleton() { return <div className="flex flex-col gap-4">{Array.from({ length: 2 }, (_, index) => <Card key={index} className="bg-background"><CardHeader><div className="flex items-start justify-between gap-3"><div className="flex min-w-0 items-start gap-3"><Skeleton className="size-4" /><div className="min-w-0 flex flex-col gap-2"><Skeleton className="h-3.5 w-40" /><Skeleton className="h-3 w-64" /></div></div><Skeleton className="h-6 w-14" /></div></CardHeader><CardContent className="flex flex-col gap-3"><div className="grid grid-cols-1 gap-3 sm:grid-cols-2"><Skeleton className="h-9 w-full max-w-xs" /><Skeleton className="h-9 w-full max-w-32" /></div><Skeleton className="h-7 w-24" /></CardContent></Card>)}</div>; }
