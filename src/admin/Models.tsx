...
export function Models({ client }: { readonly client: AdminConsoleClient }) {
  ...
  return <section className="flex flex-1 flex-col gap-4 overflow-y-auto p-4 sm:p-6"><div className="mx-auto flex w-full max-w-5xl flex-col gap-4"><div className="flex items-center justify-between gap-4"><div><h1 className="text-lg font-semibold">{translate('zh', 'models')}</h1><p className="text-sm text-muted-foreground">{translate('zh', 'modelsDescription')}</p></div><Button variant="outline" size="sm" disabled={loading} onClick={() => void load()}>{loading ? <Spinner data-icon="inline-start" /> : <RefreshCw data-icon="inline-start" />}{translate('zh', loading ? 'refreshing' : 'refresh')}</Button></div>{error ? <Alert variant="destructive"><CircleAlert aria-hidden="true" /><AlertDescription>{translate('zh', error)}</AlertDescription></Alert> : null}<ModelCreateForm client={client} onCreated={load} />{loading && !state ? <div className="flex items-center gap-2 rounded-lg border p-6 text-sm text-muted-foreground"><Spinner />{translate('zh', 'loadingResources')}</div> : null}{state ? <div className="flex flex-col gap-4"><ModelList models={state.models} assignments={state.assignments} client={client} onChanged={load} /></div> : null}</div></section>;
}
...
function ModelCreateForm({ client, onCreated }: { readonly client: AdminConsoleClient; readonly onCreated: () => Promise<void> }) {
  ...
  return <form className="flex flex-col gap-4 rounded-lg border bg-card p-4" onSubmit={submit} noValidate><div className="flex items-center justify-between gap-3"><h2 className="text-base font-semibold">{translate('zh', 'addModel')}</h2><Button type="button" variant="ghost" size="sm" onClick={() => setOpen(false)}>{translate('zh', 'cancel')}</Button></div>{error ? <Alert variant="destructive"><CircleAlert aria-hidden="true" /><AlertDescription>{translate('zh', 'modelFormFailed')}</AlertDescription></Alert> : null}<FieldGroup><Field><FieldLabel htmlFor="model-id">{translate('zh', 'modelId')}</FieldLabel><Input id="model-id" value={id} onChange={event => setId(event.target.value)} disabled={pending} /></Field><Field><FieldLabel htmlFor="model-name">{translate('zh', 'modelName')}</FieldLabel><Input id="model-name" value={displayName} onChange={event => setDisplayName(event.target.value)} disabled={pending} /></Field><Field><FieldLabel htmlFor="model-endpoint">{translate('zh', 'modelEndpoint')}</FieldLabel><Input id="model-endpoint" type="url" value={endpoint} onChange={event => setEndpoint(event.target.value)} placeholder={translate('zh', 'modelEndpointPlaceholder')} disabled={pending} /></Field><Field><FieldLabel htmlFor="model-upstream">{translate('zh', 'upstreamModel')}</FieldLabel><Input id="model-upstream" value={upstreamModel} onChange={event => setUpstreamModel(event.target.value)} disabled={pending} /></Field></FieldGroup><Button type="submit" disabled={pending}>{pending ? <Spinner data-icon="inline-start" /> : <Check data-icon="inline-start" />}{translate('zh', pending ? 'saving' : 'save')}</Button></form>;
}

function ModelList({ models, assignments, client, onChanged }: { readonly models: readonly AdminModel[]; readonly assignments: readonly ModelAssignment[]; readonly client: AdminConsoleClient; readonly onChanged: () => Promise<void> }) {
  if (models.length === 0) return <div className="flex flex-col items-center gap-2 rounded-lg border border-dashed p-8 text-center"><ShieldCheck className="size-8 text-muted-foreground" aria-hidden="true" /><span className="text-sm text-muted-foreground">{translate('zh', 'modelsEmpty')}</span></div>;
  return <div className="flex flex-col gap-4">{models.map(model => <ModelRow key={model.id} model={model} assignments={assignments.filter(item => item.resourceId === model.id)} client={client} onChanged={onChanged} />)}</div>;
}

function ModelRow({ model, assignments, client, onChanged }: { readonly model: AdminModel; readonly assignments: readonly ModelAssignment[]; readonly client: AdminConsoleClient; readonly onChanged: () => Promise<void> }) {
  return <article className="flex flex-col gap-4 rounded-lg border bg-card p-4"><div className="flex flex-wrap items-start justify-between gap-3"><div className="flex min-w-0 items-start gap-3"><Cpu className="mt-1 size-4 shrink-0 text-muted-foreground" aria-hidden="true" /><div className="min-w-0"><h2 className="truncate text-sm font-semibold">{model.displayName}</h2><p className="break-all text-xs text-muted-foreground">{model.id} / {model.upstreamModel || model.localModelRef || translate('zh', 'notProvided')}</p></div></div><div className="flex items-center gap-2"><Badge variant={model.enabled ? 'default' : 'outline'}>{translate('zh', model.enabled ? 'enabled' : 'disabled')}</Badge>{model.isDefault ? <Badge variant="outline">{translate('zh', 'defaultModel')}</Badge> : null}</div></div><dl className="grid grid-cols-1 gap-3 text-sm sm:grid-cols-2"><div><dt className="text-xs text-muted-foreground">{translate('zh', 'modelEndpoint')}</dt><dd className="break-all font-medium">{model.endpoint || translate('zh', 'notProvided')}</dd></div><div><dt className="text-xs text-muted-foreground">{translate('zh', 'assignments')}</dt><dd className="font-medium">{assignments.length}</dd></div></dl><div className="flex flex-wrap gap-2"><Button size="sm" variant="outline" onClick={async () => { await client.updateModel(model.id, { enabled: !model.enabled }); await onChanged(); }}>{translate('zh', model.enabled ? 'disable' : 'enable')}</Button>{!model.isDefault ? <Button size="sm" variant="outline" onClick={async () => { await client.updateModel(model.id, { isDefault: true }); await onChanged(); }}>{translate('zh', 'makeDefault')}</Button> : null}</div>{assignments.length > 0 ? <div className="flex flex-col gap-2 border-t pt-3">{assignments.map(assignment => <div className="flex items-center justify-between gap-3 text-sm" key={assignment.id}><span className="truncate text-muted-foreground">{assignment.subject.type}: {assignment.subject.id}</span><Button size="sm" variant="destructive" onClick={async () => { await client.deleteModelAssignment(assignment.id); await onChanged(); }}><Trash2 data-icon="inline-start" />{translate('zh', 'revoke')}</Button></div>)}</div> : null}</article>;
}
